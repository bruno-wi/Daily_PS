...
const getCntComputerWithVirus = (numOfComputer) => {
  const visitedComputer = new Array(numOfCmputers + 1).fill(false);
  const stackNodeList = [numOfComputer];
  let countOfVirusComputer = 0;

  while (stackNodeList.length) {
    let computer = stackNodeList.pop();

    if (visitedComputer[computer]) continue;
    visitedComputer[computer] = true;

    countOfVirusComputer += 1;
